...
def get_lookup_table():
    ...
    table = pd.read_csv(FILE_LOCATION, dtype={ \
                        'key_sr_nfl': object, 'key_sr_nba': object, 'key_sr_nhl': object})
    cols_to_keep = ['name_last', 'name_first', 'key_mlbam', 'key_retro',
    ...
    # Pandas cannot handle NaNs in integer columns. We need IDs to
    # be ints for successful queries in statcast, etc.
    # Workaround: replace ID NaNs with -1, then convert columns to
    #integers. User will have to understand that -1 is not a valid ID.
    table[['key_mlbam', 'key_fangraphs']] = table[[
        'key_mlbam', 'key_fangraphs']].fillna(-1)
    table[['key_mlbam', 'key_fangraphs']] = table[['key_mlbam', 'key_fangraphs']].astype(
        int)  # originally returned as floats which is wrong
    return table


def playerid_lookup(last, first=None):
    """
    Funcationality for looking up a player
    """
    last = last.lower()
    if first:
        first = first.lower()
